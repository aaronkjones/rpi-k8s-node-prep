#!/bin/bash
read -r -p "Is this a Master or Slave node (master/slave)? " node_choice
case "$node_choice" in
...
read -r -p "Do you need to install a different version? (y/n) " docker_choice
echo
...
read -r -p " Use latest Kubeadm version? (y/n) " kuber_choice
...
read -r -p "Do you want to prevent docker-ce from being upgraded? " upgrade_docker
...
read -r -p "Do you want to prevent kubeadm from being upgraded? " upgrade_kuber
